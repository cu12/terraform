...
func resourceAwsElasticTranscoderPipelineCreate(d *schema.ResourceData, meta interface{}) error {
	...
	output, err := elastictranscoderconn.CreatePipeline(req)
	if err != nil {
		return fmt.Errorf("Error creating Elastic Transcoder pipeline: %s", err)
	}

	d.SetId(*output.Pipeline.Id)
...
func resourceAwsElasticTranscoderPipelineRead(d *schema.ResourceData, meta interface{}) error {
	...
	out, err := elastictranscoderconn.ReadPipeline(&elastictranscoder.ReadPipelineInput{
		Id: aws.String(d.Id()),
	})
	...
	log.Printf("[DEBUG] Elastic Transcoder Pipeline Read output: %#v", out)
	...
	if val := pipeline.OutputBucket; val != nil {
		d.Set("output_bucket", val)
	}
	if val := pipeline.AwsKmsKeyArn; val != nil {
		d.Set("aws_kms_key_arn", val)
	}
